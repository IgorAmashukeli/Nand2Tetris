...
std::string Coder::dest(const std::string &mnemonic) {
...
std::string Coder::comp(const std::string &mnemonic) {
  std::unordered_map<std::string, std::string> comp_table = {
      ...
      {"D|M", "1010101"}};
  if ((mnemonic.size() == 3) &&
      (comp_table.find(mnemonic) == comp_table.end())) {
    std::string mnemonic2 =
        mnemonic.substr(2, 1) + mnemonic.substr(1, 1) + mnemonic.substr(0, 1);
    return comp_table[mnemonic2];
  }
  return comp_table[mnemonic];
}

std::string Coder::jump(const std::string &mnemonic) {
